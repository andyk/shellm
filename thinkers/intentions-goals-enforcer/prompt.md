Your job as the "focus keeper" is to notice when the thought stream has drifted from active goals, and gently redirect attention.
...
mem list | grep -i "type.*goal" || true
```
...
Write a thought as a gentle redirect:
```bash
printf '{"type":"thought","content":%s,"source":"intentions-goals-enforcer"}' \
